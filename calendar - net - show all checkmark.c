 ...
 *
 * With GCC (MinGW-w64):
 *   gcc -Os -s -Wl,--subsystem,windows -mwindows -o calendar.exe calendar.c -lgdi32 -lole32 -limm32 -lcomdlg32 -lcomctl32 -lws2_32
 *
...
void LoadINI() {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%63[^=]=%127[^\n]", key, val) == 2) {
            ...
            } else if (strcmp(section, "People") == 0) {
                if (strcmp(key, "Names") == 0) {
                    char* nextToken = NULL;
                    char* token = strtok_s(val, ",", &nextToken);
                    int i = 0;
                    while (token && i < 7) {
                        strncpy(aPeople[i], token, 63);
                        aPeople[i][63] = '\0';
...
void InitConfig(void) {
    ...
    // Load and/or write fallback People Names
    char tempNames[512];
    GetPrivateProfileStringA("People", "Names", "", tempNames, sizeof(tempNames), sINIFile);
    if (strlen(tempNames) == 0) {
        strcpy(tempNames, "Jack,Bob,Charlie,David,Eve,Frank,Grace");
        WritePrivateProfileStringA("People", "Names", tempNames, sINIFile);
    }
    ...
    if (iMyNodeID == 1) {
        for (int i = 0; i < MAX_SERVERS; i++) {
            if (strcmp(g_servers[i], "0") != 0 && strlen(g_servers[i]) > 0 && _IsLocalIP(g_servers[i])) {
                ...
                // Write the matched node ID back to the INI file
                char newNodeStr[16];
                snprintf(newNodeStr, sizeof(newNodeStr), "%d", iMyNodeID);
                WritePrivateProfileStringA("Window", "Node", newNodeStr, sINIFile);
...
void _MergeCsvData(const char *csvFile, const char *newCsvContent) {
    ...
    // strtok_s is safe here because "\r\n" are standard ASCII characters
    ...
    while (deltaLine) {
        ...
        if (strlen(deltaLine) > 5) {
            ...
            char id[64] = {0}, title[1024] = {0}, dt[16] = {0};
            int start = 0, dur = 0, col_val = 0, pIdx = 0, ver = 0, modBy = 0;

            // --- REFACTORED STRSTR TOKENIZATION ---
            size_t delimLen = strlen(DELIM);
            char *currentToken = copy;
            char *nextToken = NULL;
            int colIdx = 0;

            while (currentToken != NULL) {
                // Find exact multi-byte delimiter
                char *delimPtr = strstr(currentToken, DELIM);
                if (delimPtr != NULL) {
                    *delimPtr = '\0';
                    nextToken = delimPtr + delimLen;
                } else {
                    nextToken = NULL;
                }

                // Map safely extracted tokens to target variables
                switch (colIdx) {
                    case 0: strcpy(id, currentToken); break;
                    case 1: strcpy(title, currentToken); break;
                    case 2: start = atoi(currentToken); break;
                    case 3: dur = atoi(currentToken); break;
                    case 4: col_val = atoi(currentToken); break;
                    case 5: strcpy(dt, currentToken); break;
                    case 6: pIdx = atoi(currentToken); break;
                    case 7: ver = atoi(currentToken); break;
                    case 8: modBy = atoi(currentToken); break;
                }
                
                colIdx++;
                currentToken = nextToken;
            }
            // --- END REFACTORED TOKENIZATION ---
            ...
            bool found = false;
            for (int i = 0; i < numEvents; i++) {
                if (strcmp(aEvents[i].id, id) == 0) {
                    found = true;
                    if (ver >= aEvents[i].version) {
                        strncpy(aEvents[i].title, title, 1023);
                        aEvents[i].startMin = start;
                        aEvents[i].duration = dur;
                        aEvents[i].color = col_val;
                        strncpy(aEvents[i].date, dt, 15);
                        aEvents[i].personIdx = pIdx;
                        aEvents[i].version = ver;
                        aEvents[i].lastModifiedBy = modBy;
                    }
                    break;
                }
            }
...
void _ProcessNodeSpecificCleanup(const char *csvFile, int targetNodeID) {
    int dest = 0;
    for (int i = 0; i < numEvents; i++) {
        if (aEvents[i].color == 2 && aEvents[i].lastModifiedBy == targetNodeID) {
            continue;
        }
        if (dest != i) {
            aEvents[dest] = aEvents[i];
        }
        dest++;
    }
    numEvents = dest;
...
void _HandleServerClient(SOCKET clientSocket) {
    ...
    // sscanf matches exact byte sequences, so DELIM here remains perfectly safe
    if (linePtr && sscanf(linePtr, "CLIENT_SYNC_DELTA" DELIM "%d" DELIM "%d", &reqNodeID, &reqIsEmpty) >= 2) {
        ...
        while (linePtr) {
            ...
            if (strlen(linePtr) > 0 && strncmp(linePtr, "ID" DELIM, 3) != 0) {
                ...
                if (delimPtr) {
                    *delimPtr = '\0';
                    char *nextToken = delimPtr + delimLen;
                    
                    clientStates = realloc(clientStates, (clientCount + 1) * sizeof(ClientRowState));
                    strcpy(clientStates[clientCount].id, currentToken);
                    
                    // Extract second token (Version)
                    char *delimPtr2 = strstr(nextToken, DELIM);
                    if (delimPtr2) {
                        *delimPtr2 = '\0';
                    }
                    
                    clientStates[clientCount].version = nextToken ? atoi(nextToken) : 0;
                    clientCount++;
                }
                // --- END REFACTORED TOKENIZATION ---
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            if (!allowOwnData && aEvents[i].lastModifiedBy == reqNodeID) {
                continue;
            }

            int clientVer = -1;
            for (int k = 0; k < clientCount; k++) {
                if (strcmp(clientStates[k].id, aEvents[i].id) == 0) {
                    clientVer = clientStates[k].version;
                    break;
                }
            }

            if (clientVer == -1 || aEvents[i].version > clientVer) {
                char title[1024];
                strcpy(title, aEvents[i].title);
                ReplaceAll(title, "\u00A6", "%2C");
                ReplaceAll(title, "\r\n", "%0A");
                ...
                char csvLine[2048];
                snprintf(csvLine, sizeof(csvLine), "%s" DELIM "%s" DELIM "%d" DELIM "%d" DELIM "%d" DELIM "%s" DELIM "%d" DELIM "%d" DELIM "%d\n",
                        aEvents[i].id, title, aEvents[i].startMin, aEvents[i].duration,
                        aEvents[i].color, aEvents[i].date, aEvents[i].personIdx,
                        aEvents[i].version, aEvents[i].lastModifiedBy);
...
void _RunAllClientSyncs(void) {
    ...
    for (int i = 0; i < numEvents; i++) {
        if (strlen(aEvents[i].id) > 0) {
            char rowPair[128];
            snprintf(rowPair, sizeof(rowPair), "%s" DELIM "%d\n", aEvents[i].id, aEvents[i].version);
...
EnterCriticalSection(&g_csvLock);
    ...
    for (int i = 0; i < numEvents; i++) {
        ...
        if (!bShowAllTasks && (iViewMode == 1 || iViewMode == 2 || iViewMode == 3) && aEvents[i].lastModifiedBy != iMyNodeID) continue;
            ...
            if (dayNum >= 1 && dayNum <= daysInMonth) {
                ...
                for (int i = 0; i < numEvents && count < 100; i++) {
                    if (aEvents[i].color != 2 && strcmp(aEvents[i].date, cellDate) == 0) {
                        ...
                        if (!bShowAllTasks && aEvents[i].lastModifiedBy != iMyNodeID) continue;
...
int GetSortedUpcomingIndices(int* outIndices, int maxOut) {
    ...
    for (int i = 0; i < numEvents && count < 2048; i++) {
        // Find future events that aren't marked for deletion
        if (strcmp(aEvents[i].date, sCurrentDate) >= 0 && aEvents[i].color != 2) {
            
            // NEW: Filter out other nodes' events if 'Show All' is unchecked
            if (!bShowAllTasks && aEvents[i].lastModifiedBy != iMyNodeID) {
                continue;
            }
...
int GetEventColumnIndex(int eIdx) {
    if (IsPeopleView()) {
        // Replaced strcmp with DateDiffDays for resilient date matching
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show) {
    ...
    hChkShowAll = CreateWindow("BUTTON", "Show All", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 670, 15, 80, 20, hMainGUI, (HMENU)111, hInst, NULL);
    SendMessage(hChkShowAll, BM_SETCHECK, bShowAllTasks ? BST_CHECKED : BST_UNCHECKED, 0);
    
    hLblDateTitle = CreateWindow("STATIC", "", WS_CHILD | WS_VISIBLE, 760, 10, 275, 32, hMainGUI, NULL, hInst, NULL);
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_COMMAND: {
            ...
            if (id == 111) {
                bShowAllTasks = SendMessage(hChkShowAll, BM_GETCHECK, 0, 0) == BST_CHECKED;
                InvalidateRect(hCanvas, NULL, TRUE);
            }
            return 0;
        }
        case WM_KEYDOWN:
                if (wParam == VK_DELETE) {
                SendMessage(hWnd, WM_COMMAND, 110, 0); // 110 is the ID for hBtnDelete
            }
            return 0;
        case WM_DESTROY: {
...
void GetEventScreenRect(int eIdx, RECT* r) {
    ...
    if (!bShowAllTasks && (iViewMode == 1 || iViewMode == 2 || iViewMode == 3 || iViewMode == 6) && aEvents[eIdx].lastModifiedBy != iMyNodeID) return;
...
void CloseInPlaceEdit(int bSave) {
    if (iEditingIndex != -1) {
        ...
        if (bSave) {
            char newText[64];
            GetWindowTextA(hInPlaceEdit, newText, 64);
            
            // Strip out potential newlines from hitting Enter
            char* nl = strpbrk(newText, "\r\n");
            if (nl) *nl = '\0';
...
LRESULT CALLBACK CanvasWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_LBUTTONDBLCLK: {
            ...
            // Check for double clicks in the Header row to edit People names
                ...
                if (IsPeopleView()) {
                    ...
                    if (mx + iScrollX > iTimeColWidth) {
                        ...
                        if (colIdx >= 0 && colIdx < cols && colIdx < numPeople) {
                            ...
                            int hx = iTimeColWidth + (colIdx * dColW) - iScrollX;
                            SetWindowTextA(hInPlaceEdit, aPeople[colIdx]);
           ...
           case WM_KEYDOWN:
                    if (wParam == VK_DELETE) {
                        // Send the command to the parent window (MainWndProc) to process the deletion
                        SendMessage(GetParent(hWnd), WM_COMMAND, 110, 0); 
                    }
                    return 0;
            }
