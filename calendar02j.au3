...
Global $idComboView = GUICtrlCreateCombo("", 400, 13, 140, 25, $CBS_DROPDOWNLIST)
...
Global $idLblDateTitle = GUICtrlCreateLabel("", 560, 10, 470, 32)
...
; FIX: Force initial redraw after GUI is shown to ensure proper client dimensions
...
Func _CloseInPlaceEdit($bSave = True)
    If $iEditingIndex <> -1 Then
        ...
        $iEditingIndex = -1
        ; Use True to force background erasure, fixing the artifact bug where edit boxes are drawn over
        _WinAPI_InvalidateRect($hCanvas, 0, True) 
    EndIf
EndFunc
...
Func _UpdateScrollBars()
    ...
    If $iViewMode == 6 Then
        ; Disable scrolling in Month View (fits to screen)
        $iScrollY = 0
    ...
    Else
        If $iViewMode == 7 Then
            ; Dynamic scroll height for upcoming view
            Local $iCount = 0
            For $i = 0 To UBound($aEvents) - 1
                If $aEvents[$i][4] >= $sCurrentDate Then $iCount += 1
            Next
            $iCanvasHeight = ($iCount * 100) + 40
        Else
...
Func _DrawCalendar($hDC)
    ...
    ; Route to appropriate drawing routine
    If $iViewMode == 6 Then
    ...
    ; Blit buffer to screen and cleanup
...
Func _DrawTimelineView($hMemDC, $iCanvasW, $iCanvasH)
    ...
    Local $iDayColWidth = ($iEffectiveWidth - $iTimeColWidth) / $iColCount
    ...
    If _IsPeopleView() And $sCurrentDate == $sTodayStr Then
        $iRedCol = -2 ; Draw across all team columns
...
For $c = 0 To $iColCount - 1
    ...
    Local $tColRect = _WinAPI_CreateRectEx($iColLeft, 6, $iColLeft + $iDayColWidth, 25)
    Local $sHeaderText = ""
    ...
    If _IsPeopleView() Then
        ...
        EndIf
    Else
        ; Manually calculate date to avoid _DateAdd timezone/format issues
        Local $iYear = Int(StringLeft($sCurrentDate, 4))
        Local $iMonth = Int(StringMid($sCurrentDate, 6, 2))
        Local $iDay = Int(StringRight($sCurrentDate, 2))
        
        ; Add days manually
        For $d = 1 To $c
            $iDay += 1
            Local $iDaysInCurrentMonth = _DateDaysInMonth($iYear, $iMonth)
        ...
        Local $sColDate = StringFormat("%04d/%02d/%02d", $iYear, $iMonth, $iDay)
        $sHeaderText = _FormatDayHeader($sColDate)
    EndIf

    _WinAPI_DrawText($hMemDC, $sHeaderText, $tColRect, BitOR($DT_CENTER, $DT_TOP, $DT_SINGLELINE))
    If $c > 0 Then
...
Func _DrawMonthView($hMemDC, $iCanvasW, $iCanvasH)
    ...
    _GDI_SetTextColor($hMemDC, 0x3C4043)
    For $c = 0 To 6
        Local $tColRect = _WinAPI_CreateRectEx($c * $iColW, 6, ($c + 1) * $iColW, 25)
    ...
    For $row = 0 To 5
        For $col = 0 To 6
            ...
            Local $tCellRect = _WinAPI_CreateRectEx($iX1, $iY1, $iX2, $iY2)
            ...
            If $iDayNum >= 1 And $iDayNum <= $iDaysInMonth Then
                ...
                ; Render event pills with corrected positioning
                For $p = 0 To $iDayEvents - 1
                    If $p < $iMaxVisible Then
                        Local $eIdx = $aDayEventIdx[$p]
                        ; FIXED: Calculate pill position correctly with consistent padding
                        ...
                        ; FIXED: Draw rounded rect with correct brush/pen order
                        Local $hBrushEv = _GDI_CreateSolidBrush($iColor)
                        Local $hPenEv   = _GDI_CreatePen(0, 1, _DarkenColor($iColor, 15))
                        
                        _WinAPI_SelectObject($hMemDC, $hPenEv)      ; Select PEN first
                        _WinAPI_SelectObject($hMemDC, $hBrushEv)    ; Then select BRUSH
                        
                        _GDI_RoundRect($hMemDC, $iPillLeft, $iPillTop, $iPillRight, $iPillBottom, 4, 4)

                        If $eIdx <> $iEditingIndex Then
                            _GDI_SetTextColor($hMemDC, _ContrastColor($iColor)) ; Better contrast - FIXED
                            Local $tEvRect = _WinAPI_CreateRectEx($iPillLeft + 5, $iPillTop + 2, $iPillRight - 5, $iPillBottom - 2)
                    ...
                    ElseIf $p == $iMaxVisible And $iDayEvents > $iMaxVisible Then
                        ...
                        Local $tMoreRect = _WinAPI_CreateRectEx($iX1 + 6, $iY1 + 22 + ($p * 15), $iX2 - 6, $iY2)
            ...
            ; Draw cell borders (only right and bottom for clean grid)
            _WinAPI_SelectObject($hMemDC, $hPenBorder)
            _WinAPI_MoveTo($hMemDC, $iX1, $iY1)
            _WinAPI_LineTo($hMemDC, $iX2, $iY1)  ; Top border
            _WinAPI_MoveTo($hMemDC, $iX1, $iY1)
            _WinAPI_LineTo($hMemDC, $iX1, $iY2)  ; Left border
        Next
    Next
...
Func _DrawUpcomingView($hMemDC, $iCanvasW, $iCanvasH)
    _WinAPI_SetBkMode($hMemDC, $TRANSPARENT)
    Local $hFontTitle = _WinAPI_CreateFont(18, 0, 0, 0, 700, False, False, False, 0, 0, 0, 0, 0, "Segoe UI") ; <--- Larger Font
    Local $hFontText  = _WinAPI_CreateFont(14, 0, 0, 0, 400, False, False, False, 0, 0, 0, 0, 0, "Segoe UI") ; <--- Larger Font
    Local $hOldFont   = _WinAPI_SelectObject($hMemDC, $hFontTitle)
    Local $iEffectiveW = ($iCanvasW > $iCanvasWidth) ? $iCanvasW : $iCanvasW
    
    ; Setup index array mapping sorted items logic
    Local $aUpIdx[0]
    For $i = 0 To UBound($aEvents) - 1
        If $aEvents[$i][4] >= $sCurrentDate Then _ArrayAdd($aUpIdx, $i)
    Next
    
    ; Standard Date/Time Bubble sort
    ...
    For $i = 0 To UBound($aUpIdx) - 1
        ...
        If $iY + 90 > 0 And $iY < $iCanvasH Then
            ...
            ; --- Fixed Card Drawing System Instead of the Broken Dot ---
            Local $iCardLeft = 20 - $iScrollX
            Local $iCardRight = $iEffectiveW - 20 - $iScrollX
            Local $iCardTop = $iY
            Local $iCardBottom = $iY + 80
            
            ; 1) Draw Accent Edge Color Bar
            Local $hBrushEv = _GDI_CreateSolidBrush($aEvents[$e][3])
            Local $tColorRect = _WinAPI_CreateRectEx($iCardLeft, $iCardTop, $iCardLeft + 15, $iCardBottom)
            _WinAPI_FillRect($hMemDC, DllStructGetPtr($tColorRect), $hBrushEv)
            _WinAPI_DeleteObject($hBrushEv)
            
            ; 2) Draw the Card Body
            Local $hBrushBg = _WinAPI_CreateSolidBrush(0xF8F9FA)
            Local $tBgRect = _WinAPI_CreateRectEx($iCardLeft + 15, $iCardTop, $iCardRight, $iCardBottom)
            ...
            ; 4) Draw Formatted Target Text
            If $e <> $iEditingIndex Then
                _WinAPI_SelectObject($hMemDC, $hFontTitle)
                _GDI_SetTextColor($hMemDC, 0x202124)
                Local $tTitle = _WinAPI_CreateRectEx($iCardLeft + 35, $iY + 14, $iCardRight - 15, $iY + 40)
            ...
            _GDI_SetTextColor($hMemDC, 0x5F6368)
            Local $tDesc = _WinAPI_CreateRectEx($iCardLeft + 35, $iY + 45, $iCardRight - 15, $iY + 70)
            _WinAPI_DrawText($hMemDC, $sDate & "   •   " & $sTime & "   •   " & $sPerson, $tDesc, BitOR($DT_LEFT, $DT_TOP, $DT_SINGLELINE, $DT_END_ELLIPSIS))
        EndIf
        
        $iY += 100 ; Increased Vertical Loop Spacing for Larger text logic
    Next
...
Func WM_SIZE($hWnd, $iMsg, $wParam, $lParam)
    If $hWnd == $hMainGUI Then
        ...
        ; Only move canvas if we have a valid canvas handle
        If $hCanvas <> 0 Then
...
Func WM_LBUTTONDOWN($hWnd, $iMsg, $wParam, $lParam)
    ...
    For $i = UBound($aEvents) - 1 To 0 Step -1
        ...
        If $iMouseX >= $aRect[0] And $iMouseX <= $aRect[2] And $iMouseY >= $aRect[1] And $iMouseY <= $aRect[3] Then
            ...
            If $iViewMode >= 6 Then
                $iDragMode = 1 ; Move/Copy only
            Else
                If ($iMouseY - $aRect[1]) <= 6 Then
                    $iDragMode = 2 ; Resize Top
                ElseIf ($aRect[3] - $iMouseY) <= 6 Then
                    $iDragMode = 3 ; Resize Bottom
                Else
                    $iDragMode = 1 ; Move Block
...
Func WM_MOUSEMOVE($hWnd, $iMsg, $wParam, $lParam)
    ...
    ; Handle Active Dragging / Resizing / Ctrl+Copying
    ...
    For $i = UBound($aEvents) - 1 To 0 Step -1
        ...
        If $iRawX >= $aRect[0] And $iRawX <= $aRect[2] And $iRawY >= $aRect[1] And $iRawY <= $aRect[3] Then
            If $iViewMode <= 5 And (($iRawY - $aRect[1]) <= 6 Or ($aRect[3] - $iRawY) <= 6) Then
                _WinAPI_SetCursor(_WinAPI_LoadCursor(0, 32645)) ; Double Vertical Arrow
                $bOnHandle = True
            EndIf
            ExitLoop
        EndIf
...
Func WM_LBUTTONDBLCLK($hWnd, $iMsg, $wParam, $lParam)
    ...
    ; 1. Check if an existing event was double-clicked
    For $i = UBound($aEvents) - 1 To 0 Step -1
        Local $aRect = _GetEventScreenRect($i)
    ...
    ; 2. If clicked empty space -> Create new event
    ...
    If $iViewMode == 6 Then
        ...
        EndIf
    ElseIf $iViewMode == 7 Then
        Return $GUI_RUNDEFMSG ; Skip creation directly from upcoming view
...
Func WM_VSCROLL($hWnd, $iMsg, $wParam, $lParam)
    If $hWnd == $hCanvas And $iViewMode <> 6 Then
        ...
        Switch $iReq
            ...
            Case 3 ; SB_PAGEDOWN
                $iScrollY += $iVisibleH
            Case 4, 5 ; SB_THUMBTRACK, SB_THUMBPOSITION
...
Func _GetEventScreenRect($iEventIdx)
    ...
    ElseIf $iViewMode == 6 Then
        ...
        ; Count events for THIS day and find this event's pillar index (FIXED)
        Local $iDayEvents = 0
        Local $iMyPillIdx = -1
        Local $sCellDate = $aEvents[$iEventIdx][4]
        
        ; Sort by time first to match rendering order (FIXED)
        Local $aDayEventIdx[0]
        For $e = 0 To UBound($aEvents) - 1
            If $aEvents[$e][4] == $sCellDate Then _ArrayAdd($aDayEventIdx, $e)
        Next
        ...
        ; Find the index of this event in the sorted list
...
Local $iMaxVisible = Floor(($iRowH - 22) / 15)
        ...
        ; FIXED: Use matching padding from _DrawMonthView
        $aRect[0] = ($col * $iColW) + 4
    ...
    ElseIf $iViewMode == 7 Then
        ...
        For $i = 0 To UBound($aUpIdx) - 1
                If $aUpIdx[$i] == $iEventIdx Then
                    $aRect[0] = 35 - $iScrollX
                    $aRect[1] = 20 - $iScrollY + ($i * 100) + 14 ; Map inside the new box logic
...
Func _GetDateFromMonthXY($iX, $iY)
    ...
    ; CRITICAL: Must match _DrawMonthView() exactly
    Local $iStartDay = _DateToDayOfWeek($iYear, $iMonth, 1)
    Local $iCellIdx = ($row * 7) + $col + 1
...
; NEW FUNCTION: Get events for CURRENT VIEW instead of hardcoded upcoming
Func _GetEventsForCurrentView()
    ...
    Switch $iViewMode
        ...
        Case 2, 4  ; 4 Day View, 4 Person View
            For $i = 0 To UBound($aEvents) - 1
        ...
        Case 3, 5  ; Week View, 7 Person View
            For $i = 0 To UBound($aEvents) - 1
                Local $sDate = $aEvents[$i][4]
                For $d = 0 To 6
                    If $sDate == _DateAdd('d', $d, $sCurrentDate) Then 
...
; ==============================================================================
; MULTI-PAGE WIN32 GDI PRINT SYSTEM - FIXED
; ==============================================================================
Func _PrintSchedule()
    ; Step 1: Hook the OS System Printer Dialog Window
    Local $tPD = DllStructCreate("dword lStructSize;hwnd hwndOwner;handle hDevMode;handle hDevNames;hwnd hDC;dword Flags;word nFromPage;word nToPage;word nMinPage;word nMaxPage;word nCopies;handle hInstance;ptr lCustData;ptr lpfnPrintHook;ptr lpfnSetupHook;ptr lpPrintTemplateName;ptr lpSetupTemplateName;handle hPrintTemplate;handle hSetupTemplate")
    DllStructSetData($tPD, "lStructSize", DllStructGetSize($tPD))
    DllStructSetData($tPD, "hwndOwner", $hMainGUI)
    DllStructSetData($tPD, "Flags", 0x00000100) ; PD_RETURNDC (Ensures it hands us the raw Device Context ID for Drawing)

    Local $aRet = DllCall("comdlg32.dll", "bool", "PrintDlgW", "struct*", $tPD)
    If @error Or Not $aRet[0] Then Return ; Gracefully return if user clicks 'Cancel'

    Local $hPrintDC = DllStructGetData($tPD, "hDC")
    If Not $hPrintDC Then Return

    ; Step 2: Grabbing System DPI Settings limits for accurate math representation onto standard letter paper
    Local $iPageWidth  = _WinAPI_GetDeviceCaps($hPrintDC, 8) ; HORZRES Width 
    Local $iPageHeight = _WinAPI_GetDeviceCaps($hPrintDC, 10) ; VERTRES Height
    Local $iDPIY       = _WinAPI_GetDeviceCaps($hPrintDC, 90) ; LOGPIXELSY Pixels Per Inch vertical

    ; Setup docinfo stream mapping for the spooler
    ...
    DllStructSetData($tDocName, 1, "Calendar Schedule")
    ...
    ; Step 3: Get events for CURRENT VIEW instead of hardcoded upcoming
    Local $aPrintIdx = _GetEventsForCurrentView()

    ; Setup Margins and Split Pagination
    Local $iMargin = Round($iDPIY * 1) ; standard 1 inch margins all around
    Local $iLineHeight = Round($iDPIY * 26 / 72)
    Local $iItemsPerPage = Floor(($iPageHeight - (2 * $iMargin)) / ($iLineHeight * 3.5))
    If $iItemsPerPage < 1 Then $iItemsPerPage = 1

    ; Generate mapped Print DPI fonts (must pass a negative logic point height to correctly size mapping on native papers)
    Local $hFont     = _WinAPI_CreateFont(-Round($iDPIY * 11 / 72), 0, 0, 0, 400, False, False, False, 0, 0, 0, 0, 0, "Arial")
    Local $hFontBold = _WinAPI_CreateFont(-Round($iDPIY * 16 / 72), 0, 0, 0, 700, False, False, False, 0, 0, 0, 0, 0, "Arial")
    Local $hOldFont  = _WinAPI_SelectObject($hPrintDC, $hFont)

    Local $iY = $iMargin
    
    DllCall("gdi32.dll", "int", "StartPage", "handle", $hPrintDC)

    ; Step 4: Map loop text into standard GDI stream
    If UBound($aPrintIdx) == 0 Then
        _WinAPI_SelectObject($hPrintDC, $hFont)
        Local $tNoRect = _WinAPI_CreateRectEx($iMargin, $iY, $iPageWidth - $iMargin, $iY + $iLineHeight)
        _WinAPI_DrawText($hPrintDC, "No events found for current view.", $tNoRect, BitOR($DT_LEFT, $DT_TOP, $DT_SINGLELINE))
    Else
        Local $iItemCount = 0
        For $i = 0 To UBound($aPrintIdx) - 1
            If $iItemCount >= $iItemsPerPage Then
            ...
            _WinAPI_SelectObject($hPrintDC, $hFontBold)
            Local $tTitleRect = _WinAPI_CreateRectEx($iMargin, $iY, $iPageWidth - $iMargin, $iY + $iLineHeight)
            _WinAPI_DrawText($hPrintDC, $aEvents[$e][0], $tTitleRect, BitOR($DT_LEFT, $DT_TOP, $DT_SINGLELINE, $DT_END_ELLIPSIS))
            $iY += $iLineHeight
            
            _WinAPI_SelectObject($hPrintDC, $hFont)
            Local $tDescRect = _WinAPI_CreateRectEx($iMargin, $iY, $iPageWidth - $iMargin, $iY + $iLineHeight)
    ...
    _WinAPI_DeleteObject($hFontBold)
...
; ==============================================================================
; NEW HELPER: Contrast Color for Text Readability
; ==============================================================================
