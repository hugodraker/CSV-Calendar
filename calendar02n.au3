...
Func _LoadINI()
   ...
   ; $iServerNum = Int(IniRead($sINIFile, "Server", "ServerNumber", 2))
            ...
            Switch $iDragMode
                ...
                Case 2
                    ...
                    If ($iEndMin - $iCurrentMin) >= 15 Then
                        $aEvents[$iDragIndex][2] = $iCurrentMin
                        $aEvents[$iDragIndex][3] = $iEndMin - $iCurrentMin
                    EndIf

                Case 3
                    If $iCurrentMin > 1440 Then $iCurrentMin = 1440
                    If ($iCurrentMin - $iOrigStart) >= 15 Then
                        $aEvents[$iDragIndex][3] = $iCurrentMin - $iOrigStart
                    EndIf
            EndSwitch
        EndIf
...
Func _GetEventScreenRect($iEventIdx)
    ...
    If $aEvents[$iEventIdx][4] == 2 Then Return SetError(1, 0, $aRect) ; Filter out deleted items
    ...
    If $iViewMode <= 5 Then
        ...
        $aRect[1] = Round($aEvents[$iEventIdx][2] * $fZoom) - $iScrollY + $iSubHeaderH
        $aRect[2] = $aRect[0] + $iDayColW - 8
        $aRect[3] = Round(($aEvents[$iEventIdx][2] + $aEvents[$iEventIdx][3]) * $fZoom) - $iScrollY + $iSubHeaderH
        Return $aRect
            ...
            If $iEvEnd > $iStartMinPage And $iEvStart < $iEndMinPage Then
                ...
                If ($iBoxBottom - $iBoxTop) >= 4 Then
                    ...
                    If $iBoxH >= Round($iDPIY * 0.15) Then
                        ...
                        Local $sEvText = $aEvents[$i][1] & @CRLF & _MinToTimeString($aEvents[$i][2]) & " - " & _MinToTimeString($iEvEnd)
